...
class AVION:
	...
	def Actualizar(self,agregar,c,conn):
		agregar=input("desea actualizar algún elemento de la lista?")
		while agregar=="si":
			self.update()
	...
	def ExtraerData(self):
		conn=sqlite3.connect('tutorial.db')
		try:
			pais=input("Inserte país que desea averiguar:")
			data=c.execute('SELECT * FROM stuffToPlot WHERE Destino == "{}"'.format(pais))
	...
	def Eliminar(self,agregar,c,conn):
		agregar=input("desea eliminar algún item de la lista?")
		while agregar=="si":
			self.Delete()
